...
fun HueBar(
    ...
    // Update pressOffset based on currentHue when canvasSize is available
    LaunchedEffect(currentHue, canvasSize.value) {
        if (canvasSize.value.width > 0) {
            val huePosition = currentHue * canvasSize.value.width / 360f
            pressOffset.value = Offset(huePosition.coerceIn(0f, canvasSize.value.width), 0f)
        }
    }
